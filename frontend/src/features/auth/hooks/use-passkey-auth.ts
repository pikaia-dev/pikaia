...
export function useRegisterPasskey() {
  ...
  return useMutation({
    mutationFn: async (passkeyName: string) => {
      ...
      const optionsResponse = await api.post<PasskeyRegistrationOptionsResponse>(
      ...
      const verifyResponse = await api.post<PasskeyRegistrationVerifyResponse>(
...
export function useAuthenticateWithPasskey() {
  ...
  return useMutation({
    mutationFn: async (options?: { email?: string; organizationId?: string }) => {
      ...
      const optionsResponse = await api.post<PasskeyAuthenticationOptionsResponse>(
      ...
      // Step 3: Send credential to server for verification
      const verifyResponse = await api.post<PasskeyAuthenticationVerifyResponse>(
