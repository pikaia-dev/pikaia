...
export function WebhookEndpointDialog({
    ...
    const eventsByCategory = events.reduce<Record<string, WebhookEventType[]>>(
        (acc, event) => {
            const existing = acc[event.category] ?? []
            return { ...acc, [event.category]: [...existing, event] }
        },
        {}
    )
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
                <DialogHeader>
                    <DialogTitle>
                        {isEditing ? "Edit Webhook Endpoint" : "Add Webhook Endpoint"}
                    </DialogTitle>
                    <DialogDescription>
                        {isEditing
