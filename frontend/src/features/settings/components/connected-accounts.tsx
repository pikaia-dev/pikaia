...
import { useConnectProvider } from '@/features/settings/hooks/use-connect-provider'
...
const SUPPORTED_PROVIDERS: ProviderId[] = ['google', 'github']

export function ConnectedAccounts() {
  const { getConnectedAccounts, disconnectProvider } = useApi()
  const { connectGoogle, connectGitHub } = useConnectProvider()
  const queryClient = useQueryClient()

  const { data, isLoading } = useQuery({
    queryKey: queryKeys.connectedAccounts.list(),
  ...
  const disconnectMutation = useMutation({
    ...
    onSuccess: (_data, provider) => {
      toast.success(`Disconnected ${PROVIDER_CONFIG[provider as ProviderId]?.label ?? provider}`)
  ...
  const handleConnect = (provider: ProviderId) => {
    ...
    else if (provider === 'github') connectGitHub()
  }
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>Link your accounts for single sign-on and integrations</CardDescription>
      </CardHeader>
      <CardContent>
        {isLoading ? (
