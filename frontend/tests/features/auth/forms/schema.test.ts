...
import { emailSchema } from '@/features/auth/forms/schema'

describe('emailSchema', () => {
  it('accepts valid email addresses', () => {
    expect(emailSchema.safeParse({ email: 'test@example.com' }).success).toBe(true)
    expect(emailSchema.safeParse({ email: 'user@domain.org' }).success).toBe(true)
    expect(emailSchema.safeParse({ email: 'name.surname@company.co.uk' }).success).toBe(true)
  })
