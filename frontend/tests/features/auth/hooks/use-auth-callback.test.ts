...
// Mock org-api
vi.mock('@/features/auth/utils/org-api', () => ({
...
describe('useAuthCallback', () => {
  ...
  describe('when no token in URL', () => {
    it('calls onRedirectToLogin', async () => {
      renderHook(() => useAuthCallback({ onRedirectToLogin: mockOnRedirectToLogin }))
  ...
  describe('discovery token handling', () => {
    ...
    it('auto-logs in when single active organization', async () => {
      ...
      await waitFor(() => {
        expect(mockStytchClient.magicLinks.discovery.authenticate).toHaveBeenCalledWith({
          discovery_magic_links_token: 'discovery_token_123',
        })
      })

      await waitFor(() => {
        expect(mockStytchClient.discovery.intermediateSessions.exchange).toHaveBeenCalledWith({
          organization_id: 'org-1',
          session_duration_minutes: 43200, // 30 days
        })
      })

      expect(sessionStorage.setItem).toHaveBeenCalledWith('stytch_just_logged_in', 'true')
  ...
  describe('OAuth token handling', () => {
    ...
    it('processes OAuth discovery tokens', async () => {
      ...
      await waitFor(() => {
        expect(mockStytchClient.oauth.discovery.authenticate).toHaveBeenCalledWith({
  ...
  describe('invite token handling', () => {
    ...
    it('sets error on invite authentication failure', async () => {
      mockStytchClient.magicLinks.authenticate.mockRejectedValueOnce(new Error('Invite expired'))
  ...
  describe('impersonation token handling', () => {
    ...
    it('authenticates impersonation tokens', async () => {
      ...
      await waitFor(() => {
        expect(mockStytchClient.impersonation.authenticate).toHaveBeenCalledWith({
  ...
  describe('token processing guard', () => {
    it('only processes token once (React Strict Mode protection)', async () => {
      ...
      await waitFor(() => {
        expect(mockStytchClient.magicLinks.discovery.authenticate).toHaveBeenCalledTimes(1)
      })
    })
  })
})
