...
const SUPPORTED_FORMATS = Object.freeze(['jpeg', 'png', 'webp', 'avif'])
...
exports.handler = async (event) => {
  try {
    ...
    let transformer = sharp(Buffer.from(originalImage)).resize(
      validation.width,
      validation.height,
      {
        fit: sharpFit,
...
exports.originRequestHandler = async (event) => {
  try {
    ...
    request.headers['x-transform-width'] = [
      { key: 'X-Transform-Width', value: String(validation.width) },
    ]
    request.headers['x-transform-height'] = [
      { key: 'X-Transform-Height', value: String(validation.height) },
    ]
    if (params.fit) {
